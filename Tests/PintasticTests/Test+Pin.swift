...
class Test_Pin: XCTestCase {
    ...
    func testWidthAndHeight() throws {
        let pin = sibling1
            .makePin()
            .pin(width: 100, height: 50)
            .activate()

        guard let widthConstraint = pin.constraint(ofType: .width) else {
            return XCTFail()
        }

        guard let heightConstraint = pin.constraint(ofType: .height) else {
            return XCTFail()
        }
    ...
    func testSize() throws {
        let pin = sibling1
            .makePin()
            .pin(size: CGSize(width: 100, height: 50))
            .activate()
    ...
    func testSafeArea() throws {
        let pin = sibling1
            .makePin(to: parent.safeAreaLayoutGuide)
            .pinEdges()
            .activate()
    ...
    func testIdentifiers() throws {
        ...
        let leadingByIdentifier = pin.constraint(withIdentifier: MultiItemPinConstraint.leadingEdges.rawValue)
        let heightByIdentifier = pin.constraint(withIdentifier: "sibling1.height")

        XCTAssertEqual(leadingByType?.identifier, MultiItemPinConstraint.leadingEdges.rawValue)
        XCTAssertEqual(leadingByIdentifier?.identifier, MultiItemPinConstraint.leadingEdges.rawValue)
